...
- **【强制要求】**获取到相关文章的详细信息后，必须**立即**使用 shell 的 printf >> 方式写入记录。
- **【强制要求】** JSON 格式必须正确，所有英文双引号必须转换为中文全角双引号 `「` 和 `」‌`，以避免格式错误。
- **【禁止】**在内存中累积大量详细记录。
...
2. 对每个学院的 URL **严格**执行以下流程：
  ...
  2.3. 获取详细内容：
    ...
    你需要检查这些内容中是否存在英文的双引号字符，如果存在，必须将其转换为中文的全角双引号 `「` 和 `」‌`，以确保 JSON 格式的正确性。

  2.4. **立即**将每条记录以 JSON 格式追加写入 .temp/notices_{timestamp}.jsonl：
    处理完每条文章后立即执行写入（用 >> 追加，**不要用变量累积**）:
    ```bash
    printf '%s\n' '{"学院":"...", "原始链接":"...", "标题":"...", "核心摘要":"...", "发布日期":"..."}' >> .temp/notices_{timestamp}.jsonl
    ```
