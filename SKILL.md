...
对于每个根 URL，AI 应当遵循以下递归探索逻辑，**深度不超过 4 层**（根 URL 为第 0 层，最深到第 4 层子页面）：
```text
...
- **判断页面是否包含主题相关的详细信息**：
你是一个信息筛选专家。用户关注的主题是：“{topic}”。

请判断以下网页内容是否同时满足两个条件：
  > 1. 内容与主题“{topic}”直接相关（不是仅仅出现关键词，而是真正围绕该主题展开）。
  > 2. 内容包含具体的通知细节，例如：
  >   - 具体的奖项名称、项目名称、活动名称
  >   - 具体的个人姓名、团队名、组织名
  >   - 具体的日期、时间、地点
  ...
  >   - 具体的会议/期刊名称、投稿截止日
  >   - 具体的政策文件编号、条款内容
  > 如果网页内容只是泛泛介绍主题、仅列出标题列表而无详情、或者仅包含导航菜单/目录链接，请回答“不相关”。
  > 如果确实包含上述具体信息，请回答“相关”。
  > 只输出这两个词之一，不要输出其他任何内容。
  > 
  > 网页内容如下：
  > {detail_text}
